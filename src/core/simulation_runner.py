...
class SimulationRunner:
    ...
    def __init__(self, config: SimulationConfig):
        self.config = config
        # フラグをキャッシュして分岐コストを最小化
        self._stream_only = getattr(config, 'stream_trajectory_only', False)
        # 超最小出力モードでは不要な情報は保持しない
        self.trajectory_lengths = [] if not self._stream_only else None
        self.trajectory_states = [] if not self._stream_only else None
        self.total_values = [] if not self._stream_only else None
        self.step_logs = [] if not self._stream_only else None
    ...
    def run_producer_one_step(self, producer: Producer, splicer: Splicer, 
        ...
        """
        step_log = self._initialize_step_log(current_step) if not self._stream_only else None
    ...
    def _record_trajectory_and_details(self, splicer: Splicer, producer: Producer, 
        ...
        current_final_state = None if self._stream_only else splicer.get_final_state()
    ...
    def run_splicer_one_step(self, splicer: Splicer, producer: Producer) -> Any:
        ...
        if (not self.config.minimal_output) and (not self._stream_only) and result['status'] == 'success':
    ...
    def run_scheduler_one_step(self, scheduler: Scheduler, producer: Producer, splicer: Splicer, available_states: List[int]) -> Any:
        ...
        if result['status'] == 'success':
            ...
            # 結果の表示（最小限出力モードでない場合のみ）
            if (not self.config.minimal_output) and (not self._stream_only) and (worker_moves or new_groups_config):
